...
const seedRequests: PurchaseRequest[] = [
  {
    ...
    paymentTerm: "45 gün",
    dueDate: "2026-07-22",
    materialDestination: "Merkez Depo - Teknik Servis Rafı",
  ...
  },
  {
    ...
    paymentTerm: "Peşin",
    dueDate: "2026-07-18",
    materialDestination: "İstanbul Şube - Saha mutfak kurulum alanı",
  ...
  },
  {
    ...
    paymentTerm: "20 gün",
    dueDate: "2026-07-14",
    materialDestination: "Ankara Depo - Ambalaj stok alanı",
  ...
  },
  {
    ...
    paymentTerm: "30 gün",
    dueDate: "2026-07-26",
    materialDestination: "İzmir Şube - BT teslim noktası",
      ...
      <main className="pb-24 md:ml-72 md:pb-0">
        <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 px-4 py-3 backdrop-blur md:px-8">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <div className="flex items-center gap-2 md:hidden"><Image src="/byks-logo.jpg" alt="BYKS" width={290} height={122} className="h-8 w-auto rounded bg-white object-contain" /><span className="text-lg font-bold text-navy-900">Manager</span></div>
    ...
    <main className="grid min-h-screen place-items-center bg-slate-100 p-4">
      <section className="panel w-full max-w-md p-6">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-navy-900">BYKS Manager</h1>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        <MiniChart title="Şube bazlı satın alma" rows={metrics.byBranch} />
    ...
    <aside className="panel p-4">
      ...
      <Block title="Mail taslağı">
        {order.emailDraft ? (
          ...
          <p className="text-sm text-slate-500">Manuel son kontrol gereken PO. Gönderim öncesi Cem veya yetkili kullanıcı kontrol eder.</p>
        )}
      </Block>
      ...
      {!canSend && order.status === "Gönderilmeye hazır" && (
        <p className="mt-3 text-sm text-amber-700">Bu PO için gönderim yetkisi mevcut kullanıcıda değil veya Cem manuel son kontrolü gerekiyor.</p>
      )}
    </aside>
  );
}
...
function NewRequest({ onCreate }: { onCreate: (request: PurchaseRequest) => void }) {
  const [form, setForm] = useState({
    requester: "Personel", branch: "Merkez Depo", productName: "", brandModel: "", quantity: 1, unit: "Adet", unitPrice: 0, vatIncluded: true,
    currency: "TRY", exchangeRate: 1, exchangeRateDate: new Date().toISOString().slice(0, 10), supplier: "", paymentTerm: "30 gün", dueDate: "",
    materialDestination: "Merkez Depo teslimat alanı", purpose: "", projectCustomer: "", currentStock: 0, lastPurchasePrice: 0, alternativeOffer: 0, fileName: "", notes: ""
  });
...
const textFields: [string, string, string?][] = [
  ...
  ["supplier", "Tedarikçi"], ["paymentTerm", "Vade"], ["dueDate", "Termin tarihi", "date"], ["materialDestination", "Malzemenin gideceği yer"], ["purpose", "Kullanım amacı"], ["projectCustomer", "Proje / müşteri"],
...
function roleText(role: Role) {
  return {
    ...
    Personel: "Satın alma talebi açar ve revizyonları takip eder.",
